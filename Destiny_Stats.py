    ...
    for character in profile.char_ids:
        ...
        try:
            ...
            while 'activities' in stats and stats['activities'][0]['period'] > start_search_on:
                page = page + 1
                num = 0
                while num < 5 and stats['activities'][num]['period'] > start_search_on and stats['activities'][num][
                    'period'] < stop_search_on:
                    win_dict = {}
                    ...
                    win_dict['duration'] = stats['activities'][num]['values']['activityDurationSeconds']['basic'][
                        'value']
                    win_list.append(win_dict)
                    print(win_dict)
                    num = num + 1
                stats = await get_historical_activitys(session, profile.user_id, console, character, '5', str(page),
                                                       '19')
...
async def build_trials_team(user1, user2, user3=None, console='steam', season=current_season, week=None,
                            tz="US/Pacific"):
...
def find_reset_scope(game_period):
    """Return a 2 date objects one the start of a reset the second the end of it"""
    game_datetime = parser.parse(game_period)

    print(game_datetime)
    print(game_datetime.hour)
    if game_datetime.hour > reset_time_hours:
        today = game_datetime.replace(hour=0, minute=0, second=0, microsecond=0)
    else:
        today = game_datetime.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=1)

    if today.weekday() == 1:
        offset = 0
    else:
        offset = (today.weekday() - 1) % 7
    reset = {'Prev': today - timedelta(days=offset) + timedelta(hours=reset_time_hours)}
    reset['Next'] = reset['Prev'] + timedelta(days=7)
    return reset
...
def pushto_excel_trials(trials_build, time_start, time_stop, savename, tz):
    ...
    ws['L1'] = tz
    game_col = 6
    last_reset = find_reset_scope(trials_build[0]['period'])
    for game in trials_build:
        ws['A' + str(game_col)] = game_col - 5
        game_date = parser.parse(game['period']).astimezone(timezone(tz))
        next_reset = find_reset_scope(game['period'])
    ...
    for character in profile.char_ids:
        ...
        try:
            ...
            while 'activities' in stats and stats['activities'][0]['period'] > season_start:
                page = page + 1
                num = 0
                while num < 5 and stats['activities'][num]['period'] > season_start and stats['activities'][num][
                    'period'] < season_end:
                    win_dict = {}
                    win_dict['period'] = stats['activities'][num]['period']
                    win_dict['win/loss'] = stats['activities'][num]['values']['standing']['basic']['displayValue']
...
if __name__ == '__main__':
    ...
    print("debug")
    print(find_reset_scope("2021-09-11T06:16:36Z"))
